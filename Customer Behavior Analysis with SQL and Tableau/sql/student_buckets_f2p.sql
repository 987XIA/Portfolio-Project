WITH table_period_to_consider AS (
    ...
    SELECT 
        ...
        i.date_registered,
        1 AS paid,
        MIN(p.date_purchased) AS last_date_to_watch
    FROM student_info i
    JOIN student_purchases p USING (student_id)
    GROUP BY p.student_id, i.date_registered  -- Add i.date_registered here
),
table_minutes_summed_1 AS (
    ...
    GROUP BY l.student_id, p.date_registered, p.paid, p.last_date_to_watch  -- Add columns from SELECT
),
