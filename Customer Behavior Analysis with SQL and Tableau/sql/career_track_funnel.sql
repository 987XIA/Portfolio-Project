...
( -- Get the successful course exams
	SELECT 
        ...
        CASE 
			...
			WHEN cc.course_id IS NOT NULL AND c.attempted_course_exam IS NOT NULL THEN 1
	...
	( -- Get the enrollment in track and the attempted_course_exam column
		...
		FROM (-- Get the course id's of the course exams that a student has attempted
			SELECT DISTINCT *
			FROM
	...
	LEFT JOIN career_track_info AS b ON a.track_id = b.track_id AND a.course_id = b.course_id) AS c
LEFT JOIN table_course_certificates cc ON c.student_id = cc.student_id AND c.course_id = cc.course_id ) AS d
...
(
	...
	FROM
        ...
        LEFT JOIN table_track_exams ex ON i.student_id = ex.student_id AND 
			i.enrolled_in_track_id = ex.track_id
),
...
(
    ...
    LEFT JOIN table_certificates c ON e.student_id = c.student_id AND e.enrolled_in_track_id = c.track_id
),
